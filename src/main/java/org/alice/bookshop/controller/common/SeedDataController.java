...
public class SeedDataController {
	...
	public String seedData() {
		for (int i = 1; i <= 400; i++) {
			...
			book.setTotalPages(50 + i);
			bJpa.save(book);

			Input input = new Input();
			input.setInputDate(new Date());
			input.setName("input event " + i);
			iJpa.save(input);
			...
			Bill bill = new Bill();
			bill.setBillDate(new Date(115 +(int) Math.floor(Math.random() * 5), (int) Math.floor(Math.random() * 12),
					(int) Math.floor(Math.random() * 28)));
			bill.setUser(user);
			bill.setNote("delivery to " + user.getAddress());
			bill.setState(1);
			billJpa.save(bill);
			...
			for (int j = 1; j <= 4; j++) {
				...
				billDetail.getBook().setId((int) Math.floor(Math.random() * 200));
				;
				billDetail.setQuantity((int) Math.floor(Math.random() * 10));
			}
